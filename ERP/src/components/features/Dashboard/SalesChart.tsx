// Componente de gráfico de vendas (placeholder para futuro)
// Preparado para integração com bibliotecas de gráficos
  ...
  return (
    <div>
          ...
          return (
            <div key={month} className="flex-1 flex flex-col items-center gap-2">
              <div className="w-full flex items-end justify-center h-40">
                <div
                  className="w-full bg-gradient-to-t from-red-600 to-red-500 rounded-t-lg transition-all hover:opacity-80 cursor-pointer relative group"
                  style={{ height: `${height}%` }}
                >
      ...
      <div className="mt-6 pt-4 border-t border-zinc-700 flex items-center justify-between text-sm">
        <div>
          <span className="text-zinc-400">Média mensal: </span>
          <span className="font-semibold text-white">
            R$ {(values.reduce((a, b) => a + b, 0) / values.length / 1000).toFixed(0)}K
          </span>
        </div>
        <div>
          <span className="text-zinc-400">Total no período: </span>
          <span className="font-semibold text-white">
